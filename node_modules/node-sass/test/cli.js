  ...
  describe('node-sass sass/ --output css/', function() {
    ...
    it('should compile all files in the folder', function(done) {
      ...
      bin.once('close', function() {
        ...
        assert.deepEqual(nestedFiles, ['tree.css']);
        rimraf.sync(dest);
        done();
      });
    });
    ...
    it('should compile with --source-map set to directory', function(done) {
      ...
      bin.once('close', function() {
        var map = JSON.parse(read(fixture('input-directory/map/nested/tree.css.map'), 'utf8'));

        assert.equal(map.file, '../../css/nested/tree.css');
        rimraf.sync(dest);
        rimraf.sync(destMap);
        done();
      });
